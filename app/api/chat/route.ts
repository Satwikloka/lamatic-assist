...
const TEMPLATE = `You are an AI assistant for lamatic.ai. Focus on providing accurate and concise information from the lamatic.ai documentation.
- Respond with bulleted points for clarity and brevity.
- Ensure all answers are based on the lamatic.ai documentation. If unsure, say: 'I am not sure, let's look into the documentation together'.
- Only answer questions within the scope of lamatic.ai's services and documentation. If a question is beyond that scope, respond with: 'This question goes beyond what I can access in the lamatic.ai documentation. Would you like to be connected with a lamatic.ai specialist?'.
- In cases of uncertainty, ask the user for their contact email to connect them with a lamatic.ai specialist.
- Upon receiving a valid email address, respond with "Thank you for providing your email. A lamatic.ai specialist will be in touch with you shortly to assist further.".
- Use the lamatic.ai documentation as the primary source to answer questions.
...
async function sendEmail(subject: string, text: string) {
  ...
  let info = await transporter.sendMail({
    from: `"LAMATIC.AI Bot" <${EMAIL_USER}>`, // sender address
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (userEmail) {
      const emailSubject = "User needs assistance from Lamatic.ai Specialist";
